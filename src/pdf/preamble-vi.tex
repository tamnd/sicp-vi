...
\setotherlanguages{vietnamese,greek}
...
% MathJax-style \lt and \gt aliases (the book chapter texis use them
% directly; the en_US PDF dodges this because it builds from a separate
% src/pdf/sicp.texi that has < and >). Use \AtBeginDocument so we win
% against any redefinition pulled in by newtxmath/polyglossia.
\AtBeginDocument{\def\lt{<}\def\gt{>}}
...
\hypersetup{
  ...
  pdftitle={Cấu Trúc và Diễn Giải Chương Trình Máy Tính, Tái Bản Lần 2},
  pdfsubject={khoa học máy tính, lập trình, trừu tượng hóa},
  pdflang={vi},
  colorlinks=true,
  linkcolor=LinkRed,
  urlcolor=LinkRed,
